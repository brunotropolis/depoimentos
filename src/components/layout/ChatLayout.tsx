...
const directStatusLabel = (status?: string, layoutId?: LayoutId) => {
  if (layoutId === "instagram") {
    ...
    if (status === "online") return "Ativo(a) agora"
    if (status === "empty") return ""
    return "Ativo(a) recentemente"
  }
  if (status === "typing") return "digitando…"
...
export const ChatLayout = ({
  ...
  const headerParticipant = !isGroup
    ...
    : undefined
  const title = isGroup ? getConversationTitle(conversation) : headerParticipant?.name ?? "Nova conversa"
