    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-xs text-slate-500">
          {message.type === "image" ? "imagem" : message.type === "system" ? "aviso" : "texto"} - {formatTimestamp(message.timestamp)}
        </div>
      </div>
...
export const ConversationBuilder = () => {
  ...
  const handleEasyApply = () => {
    ...
    if (!activeParticipantId || !receiverId) {
      setEasyError("Adicione pelo menos dois participantes para usar o modo rápido.")
      showToast("Adicione pelo menos dois participantes para usar o modo rápido.", "error")
      return
    }
    ...
    if (hasInvalidLine || entries.length === 0) {
      setEasyError("Use < ou > no começo de cada linha de mensagem.")
      showToast("Use < ou > no começo de cada linha de mensagem.", "error")
      return
    }
  ...
  return (
    <TooltipProvider>
      <div className="space-y-4">
        <div>
          <h3 className="text-sm font-semibold text-slate-900">Editor de conversa</h3>
          <p className="text-xs text-slate-500">
            Adicione mensagens, arraste para reordenar ou use o modo Rápido para escrever tudo de uma vez.
          </p>
        </div>

        <div className="space-y-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h4 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Mensagens</h4>
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-xs text-slate-500">{messages.length} no total</span>
              <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2">
                <Label className="text-[10px] uppercase text-slate-400">Data geral</Label>
                <Input
                  type="date"
                  value={globalDate}
                ...
                <span className="text-xs text-slate-400">
                  {hasMixedDates ? "Datas diferentes" : "Mantém o horário"}
                </span>
              </div>
              <Button
                type="button"
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2">
            <div className="space-y-1">
              <div className="text-xs font-semibold uppercase text-slate-400">Modo do editor</div>
              <p className="text-xs text-slate-500">Alterne entre a lista e o editor de texto rápido.</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Button
                type="button"
          ...
          {viewMode === "easy" ? (
            <div className="rounded-xl border border-slate-200 bg-white p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <Label className="text-xs uppercase text-slate-400">Editor rápido</Label>
                <Button
                  type="button"
                  variant="ghost"
                  size="sm"
              ...
              <div className="mt-3 space-y-2">
                <Textarea
                  ...
                  placeholder={`< mensagem de ${activeParticipant?.name ?? "Remetente"}\n> mensagem de ${receiverParticipant?.name ?? "Destinatário"}`}
                  className="min-h-[280px] resize-y"
                />
                <p className="text-xs text-slate-500">
                  <span className="font-semibold">&lt;</span> = {activeParticipant?.name ?? "Remetente"},{" "}
                  <span className="font-semibold">&gt;</span> = {receiverParticipant?.name ?? "Destinatário"}.
                  Cada linha vira uma mensagem; o horário padrão é o de agora.
                </p>
                {easyError ? (
                          ...
                          {isActionsOpen ? (
                            <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2">
                              ...
                              >
                                ...
                                {message.isHidden ? "Mostrar no chat" : "Ocultar do chat"}
                              </Button>
                              <Button
                                variant="ghost"
                                size="sm"
        ...
        {viewMode === "standard" ? (
          <>
            ...
            <div className="space-y-3">
              {isAddOpen ? (
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                  <MessageForm
                    ...
                    submitLabel="Adicionar mensagem"
                  />
                </div>
              ) : null}
              <Button
              ...
              >
                {isAddOpen ? "Fechar" : "Adicionar mensagem"}
              </Button>
            </div>
          </>
        ) : null}
